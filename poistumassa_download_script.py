...
soup = BeautifulSoup(
    requests.get("https://areena.yle.fi/tv/ohjelmat/kaikki?t=viela_ehdit").text,
    "html.parser",
)
...
for item in soup.find_all("div", {"class": "package-view"}):
    ...
    for item in cont[0]["content"]:
        ...
        if (
            not ("https://areena.api.yle.fi/v1/ui/content/list?client=yle-areena-web")
            in uri
        ):
            continue
...
def requ():
    ...
    response = requests.get(
        "https://areena.api.yle.fi/v1/ui/content/list", params=params
    )
